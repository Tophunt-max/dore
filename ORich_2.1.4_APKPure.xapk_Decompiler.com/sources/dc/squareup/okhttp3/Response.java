...
public final class Response implements Closeable {
    ...
    public ResponseBody peekBody(long j) throws IOException {
        ...
        Buffer bufferM15clone = bufferedSourceSource.buffer().m15clone();
        if (bufferM15clone.size() > j) {
            Buffer buffer = new Buffer();
            buffer.write(bufferM15clone, j);
            bufferM15clone.clear();
            bufferM15clone = buffer;
        }
        return ResponseBody.create(this.body.contentType(), bufferM15clone.size(), bufferM15clone);
    }
