...
public class GifAnimationMetaData implements Serializable, Parcelable {
    ...
    public long getDrawableAllocationByteCount(GifDrawable gifDrawable, int i) {
        int frameByteCount;
        if (i < 1 || i > 65535) {
            throw new IllegalStateException("Sample size " + i + " out of range <1, " + CharCompanionObject.MAX_VALUE + Operators.G);
        }
        int i2 = i * i;
        if (gifDrawable != null && !gifDrawable.mBuffer.isRecycled()) {
            if (Build.VERSION.SDK_INT >= 19) {
                frameByteCount = gifDrawable.mBuffer.getAllocationByteCount();
            } else {
                frameByteCount = gifDrawable.getFrameByteCount();
            }
        } else {
            frameByteCount = ((this.mWidth * this.mHeight) * 4) / i2;
        }
        return (this.mPixelsBytesCount / ((long) i2)) + ((long) frameByteCount);
    }
