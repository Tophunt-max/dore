...
public class Operators {
    ...
    public static boolean isEquals(Token token, Token token2, Object obj) {
        ...
        if (objExecute2 == null) {
            return isEmpty(objExecute.toString());
        }
        if (objExecute instanceof Number) {
            return objExecute2 instanceof Number ? ((Number) objExecute).doubleValue() == ((Number) objExecute2).doubleValue() : ((Number) objExecute).doubleValue() == getNumber(objExecute2);
        }
        if (objExecute2 instanceof Number) {
            return getNumber(objExecute) == ((Number) objExecute2).doubleValue();
        }
        if ((objExecute instanceof CharSequence) || (objExecute2 instanceof CharSequence)) {
            return objExecute.toString().trim().equals(objExecute2.toString().trim());
        }
        return objExecute.equals(objExecute2);
    }
