        ...
        public void doDecode(EncodedImage encodedImage, int status) {
            ...
            if ((encodedImage.getImageFormat() == DefaultImageFormats.JPEG || !isNotLast(status)) && !isFinished() && EncodedImage.isValid(encodedImage)) {
                ...
                try {
                    ...
                    try {
                        try {
                            closeableImageInternalDecode = internalDecode(encodedImage, size, qualityInfo2);
                        } catch (DecodeException e) {
                            EncodedImage encodedImage2 = e.getEncodedImage();
                            FLog.w("ProgressiveDecoder", "%s, {uri: %s, firstEncodedBytes: %s, length: %d}", e.getMessage(), strValueOf2, encodedImage2.getFirstBytesAsHexString(10), Integer.valueOf(encodedImage2.getSize()));
                            throw e;
                        }
                    } catch (Exception e2) {
                        e = e2;
