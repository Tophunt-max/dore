...
onShow(async () => {
  const res = await api.get('/api/withdraw/records');
...
<template>
  <view class="page">
    <view v-for="r in records" :key="r.id" class="row card">
      <view class="l">
        <text class="t">Withdrawal{{ r.holder_name ? ' · ' + r.holder_name : '' }}</text>
    ...
    <view v-if="!records.length" class="nodata">No withdrawal records</view>
  </view>
</template>
<style scoped>
