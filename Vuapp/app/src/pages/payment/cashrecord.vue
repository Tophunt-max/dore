<template>
  <view class="payrecord">
    <navbar :title="$t('payment.cashtitle')" background="#fff"></navbar>
    ...
    >
      <view v-for="(item, index) in dataList" :key="index" class="payrecord_list">
        ...
        <view class="payrecord_list_item border">
          <view class="payrecord_list_item_title">{{ $t('payment.orderno2') }}</view>
        ...
        <view class="payrecord_list_item">
          <view class="payrecord_list_item_title">{{ $t('payment.payamount2') }}</view>
...
import { cashList } from '@/api/orich';
