...
public class WXUtils {
    ...
    private static float floatByViewport(Object obj, float f) {
        ...
        if (strTrim.endsWith("wx")) {
            ...
            if (strTrim.length() > 3 && (strTrim.endsWith("rpx") || strTrim.endsWith("upx"))) {
                try {
    ...
    private static float transferWx(String str, float f) {
        ...
        return ((WXEnvironment.sApplication.getResources().getDisplayMetrics().density * Float.valueOf(Float.parseFloat(str)).floatValue()) * f) / WXViewUtils.getScreenWidth();
    }
        ...
        if (TextUtils.equals("wx", strSubstring)) {
            ...
            if (strTrim.length() > 3 && (strTrim.endsWith("rpx") || strTrim.endsWith("upx"))) {
                ...
                try {
                    ...
                    if (!TextUtils.isEmpty(strSubstring2) && strSubstring2.contains(Operators.DOT_STR)) {
                        obj = Integer.valueOf((int) parseFloat(strSubstring2));
                    } else {
                        obj = Integer.valueOf(Integer.parseInt(strSubstring2));
                    }
                } catch (NumberFormatException e5) {
                    WXLogUtils.e("Argument format error! value is " + obj, e5);
                    obj = num;
            ...
            if (!TextUtils.isEmpty(strTrim)) {
                if (strTrim.contains(Operators.DOT_STR)) {
                    obj = Integer.valueOf((int) parseFloat(strTrim));
                } else {
                    obj = Integer.valueOf(Integer.parseInt(strTrim));
                }
            } else {
                if (WXEnvironment.isApkDebugable()) {
    ...
    public static int parseUnitOrPercent(String str, int i) {
        ...
        if (iLastIndexOf != -1) {
            return parsePercent(str.substring(0, iLastIndexOf), i);
        }
        return parseInt(str);
    }
