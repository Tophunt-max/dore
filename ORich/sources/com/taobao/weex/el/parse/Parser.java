...
public class Parser {
    ...
    final void scanArray() {
        ...
        if (arrayList.size() == 1) {
            block = (Token) arrayList.get(0);
        } else {
            block = new Block(arrayList, 6);
        }
    ...
    void doOperator(Symbol symbol) {
        ...
        int i = symbol.pos;
        int iMax = Math.max(symbol.pos - 1, 0);
    ...
    final void scanNumber() {
        ...
        if (z) {
            token = new Token(strSubstring, 1);
        } else {
            token = new Token(strSubstring, 2);
        }
    ...
    final void scanIdentifier() {
        ...
        int i2 = 0;
        if (Operators.KEYWORDS.containsKey(strSubstring) && (this.operators.isEmpty() || !Operators.isDot(this.operators.peek().op))) {
            i2 = 4;
        }
        this.stacks.push(new Token(strSubstring, i2));
    }
