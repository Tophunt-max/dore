...
public class PickerActivity extends FragmentActivity implements DataCallback, View.OnClickListener {
    ...
    public void lambda$done$0$PickerActivity(final ArrayList<Media> arrayList) {
        ...
        if (next.extension.equalsIgnoreCase(".png") || next.extension.equalsIgnoreCase(".jpg") || next.extension.equalsIgnoreCase(".jpeg")) {
            String strCompressImage = CompressUtil.compressImage(next.path, this.docPath + "uniapp_temp/compressed/" + System.currentTimeMillis() + "_" + next.path.split(Operators.DIV)[r1.length - 1], next.extension.equalsIgnoreCase(".png"));
        ...
        if (this.docPath.endsWith(Operators.DIV)) {
    ...
    public void done(final ArrayList<Media> arrayList) {
        ...
        ThreadPool.self().addThreadTask(new Runnable() { // from class: com.dmcbig.mediapicker.-$$Lambda$PickerActivity$uLMNYXk_LpF_IZeoPcjCsERTDuE
            @Override // java.lang.Runnable
            public final void run() {
                this.f$0.lambda$done$0$PickerActivity(arrayList);
            }
        });
    }
    ...
    void setButtonText() {
        ...
        if (intExtra == Integer.MAX_VALUE) {
            ...
            this.done.setText(this.doneBtnText + Operators.BRACKET_START_STR + this.gridAdapter.getSelectMedias().size() + Operators.DIV + intExtra + Operators.BRACKET_END_STR);
        }
        if (!PdrUtil.isEmpty(this.cropOptions)) {
            this.done.setVisibility(4);
            findViewById(MediaPickerR.MP_ID_PREVIEW).setVisibility(4);
        ...
        this.gridAdapter.setOnItemClickListener(new MediaGridAdapter.OnRecyclerViewItemClickListener() { // from class: com.dmcbig.mediapicker.PickerActivity.3
            ...
            public void onItemClick(View view, Media media, ArrayList<Media> arrayList2) {
                ...
                if (PdrUtil.isEmpty(pickerActivity.cropOptions)) {
                    ...
                    PickerActivity.this.startActivityForResult(intent, ItemTouchHelper.Callback.DEFAULT_DRAG_ANIMATION_DURATION);
                    return;
                }
