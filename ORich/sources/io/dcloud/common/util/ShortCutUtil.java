    ...
    public static String getSreamAppShortName(Context context) {
        try {
            ...
            if (uriFromLauncher != null) {
                Cursor cursorQuery = contentResolver.query(uriFromLauncher, new String[]{AbsoluteConst.JSON_KEY_TITLE, "intent"}, null, null, null);
                if (cursorQuery != null && cursorQuery.getCount() > 0) {
                    while (cursorQuery.moveToNext()) {
                        String string = cursorQuery.getString(cursorQuery.getColumnIndex("intent"));
                        if (!TextUtils.isEmpty(cursorQuery.getString(cursorQuery.getColumnIndex(AbsoluteConst.JSON_KEY_TITLE)))) {
        ...
        new Thread() { // from class: io.dcloud.common.util.ShortCutUtil.1
            ...
            public void run() {
                ...
                if (z) {
                    ...
                    str2 = str3 + "&sc=" + (ShortCutUtil.SHORT_CUT_EXISTING.equals(strRequestShortCutForCommit) ? "s" : ShortCutUtil.SHORT_CUT_NONE.equals(strRequestShortCutForCommit) ? "n" : "u");
                }
                try {
